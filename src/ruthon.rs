...
pub fn main() {
    ...
    let test = include_str!(concat!(env!("CARGO_MANIFEST_DIR"), "/pyapp/test.py"));
    let test1 = include_str!(concat!(env!("CARGO_MANIFEST_DIR"), "/pyapp/test1.py"));

    let files = vec!["Delta.csv", "testset.csv"];
    let a = files
        .into_par_iter()
        .map(|s|{
            let from_python = Python::with_gil(|py|-> PyResult<Vec<Series>> {
                // This line will allow us to use test1 inside test
                PyModule::from_code(py, test1, "", "test1")?;
                let app: Py<PyAny> = PyModule::from_code(py, test, "", "")?
                    .getattr("s3_csv_to_lst_srs")?
                    .into();
            
                let r: Vec<Py<PyAny>> = app.call1(py, ("ultima-bi", s) )?.extract(py)?;
